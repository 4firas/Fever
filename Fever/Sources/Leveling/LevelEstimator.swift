import simd

/// IMU-free gravity/up estimation — the core of the leveling ("Body Stabilizer")
/// system that makes tracking correct regardless of camera pitch/roll.
///
/// The reference is the FOOT-DERIVED vertical: `midHip − footMid`. Feet are
/// nearly directly below the hips regardless of shoulder/head posture (unlike
/// the spine, which has natural forward lean that would otherwise bake a
/// persistent pitch into the datum). The leveling rotation forces this
/// foot-to-hip vector onto +Y without encoding the user's actual lean.
///
/// Yaw is *never* applied: a vertical foot-to-hip vector is yaw-invariant, so
/// a user who is merely turned yields identity and their body yaw flows untouched
/// through to the hip frame, the green reference box, and the OSC wire.
///
/// All inputs are in the SOLVER FRAME (+X right, +Y up, +Z toward the camera),
/// read AFTER `MediaPipeFrame`'s Y-negate and BEFORE the origin/floor latch.
/// Pure value type, no state — the per-session datum and smoothing live in
/// `BodyStabilizer`.
public enum LevelEstimator {

    /// A foot-to-hip vector shorter than this (meters) is too ill-conditioned
    /// to level from; callers get identity rather than a noise-amplified rotation.
    public static let minSpineLength: Float = 0.2

    /// The no-rotation quaternion (w = 1).
    public static let identity = simd_quatf(ix: 0, iy: 0, iz: 0, r: 1)

    /// The leveling rotation that brings `midHip − footMid` (foot-to-hip = "up")
    /// onto vertical (+Y).
    ///
    /// - pitch: rotate about +X to zero the up-vector's Z component (removes
    ///   camera up/down tilt — the primary IMU-free correction).
    /// - roll (only when `includeRoll`): rotate about +Z to zero the X component
    ///   on the already-pitched vector (removes camera roll).
    ///
    /// The result is a PROPER rotation (det +1) composed solely of axis rotations,
    /// so it must never be folded into `CoordinateMapper`'s reflection. Returns
    /// identity for an ill-conditioned (too short) up vector.
    public static func levelingQuaternion(midHip: SIMD3<Float>,
                                          footMid: SIMD3<Float>,
                                          includeRoll: Bool) -> simd_quatf {
        let spine = midHip - footMid   // foot → hip = "up"
        let len = simd_length(spine)
        guard len > minSpineLength else { return identity }
        let s = spine / len

        // Pitch about +X so the up vector lies in the X-Y plane (z' = 0).
        // Rx(a): z' = y·sin a + z·cos a = 0  ⇒  a = atan2(-z, y).
        let pitch = atan2(-s.z, s.y)
        let qPitch = simd_quatf(angle: pitch, axis: SIMD3<Float>(1, 0, 0))
        guard includeRoll else { return simd_normalize(qPitch) }

        // Roll about +Z so the already-pitched up vector is vertical in X too (x' = 0).
        // Rz(b): x' = x·cos b − y·sin b = 0  ⇒  b = atan2(x, y), on the pitched vector.
        let s1 = qPitch.act(s)
        let roll = atan2(s1.x, s1.y)
        let qRoll = simd_quatf(angle: roll, axis: SIMD3<Float>(0, 0, 1))
        return simd_normalize(qRoll * qPitch)
    }

    /// Upright sanity gate: the leveled reference is only valid when the user is
    /// actually standing — head above the hips AND at least one foot below the
    /// hips. When false the caller treats the reference as LOST (the green box
    /// vanishes and the datum is held) rather than fabricating leveling from a
    /// crouch close to the lens — the box-vanish behavior seen in the teardown.
    public static func uprightSanity(nose: SIMD3<Float>,
                                     midHip: SIMD3<Float>,
                                     leftAnkle: SIMD3<Float>,
                                     rightAnkle: SIMD3<Float>) -> Bool {
        let headAboveHips = nose.y > midHip.y
        let aFootBelowHips = leftAnkle.y < midHip.y || rightAnkle.y < midHip.y
        return headAboveHips && aFootBelowHips
    }
}
